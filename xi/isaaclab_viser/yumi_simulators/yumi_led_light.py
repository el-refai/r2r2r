...
class LedLight(IsaacLabViser):
    def __init__(self, *args, **kwargs):
        self.debug_marker_vis = False
        self.dig_config_path = Path('/home/yujustin/outputs/led_light2/dig/2025-04-07_130425/config.yml')
    ...
    def _reset_object_state(self):
        ...
        random_xyz = torch.randn_like(self.scene.env_origins) * 0.05
        random_xyz[:, 2] = 0.0
        
        z_rot = torch.rand((self.scene.num_envs,)) * 1.1 + onp.pi/2 - 0.4 #* 0.8 + onp.pi/2 - 0.4
        ...
        for group_idx, rigid_object in enumerate(self.scene.rigid_objects.values()):
            ...
            #     new_starts = generate_directional_starts(self.part_deltas[:, group_idx], self.scene.num_envs, magnitude=0.12, direction_weight=0.5, perp_variation=0.22)
            ...
            # current_transform = (tf.SE3(wxyz_xyz=self.T_world_objinit) @ 
            #                 tf.SE3(wxyz_xyz=self.T_objreg_objinit))
